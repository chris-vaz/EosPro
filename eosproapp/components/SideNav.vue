    ...
    <aside class="bg-[#1C1C1F] h-screen flex flex-col transition-all duration-200 ease-out relative"
        ...
        <NuxtLink to="/edit"
            ...
            <div class="flex items-center">
                ...
                <span v-if="isExpanded" class="ml-3 text-base leading-6">Editor</span>
            </div>
            <div v-if="!isExpanded" class="absolute top-2 right-2 w-3 h-3 rounded-full bg-[#0075DC]"></div>
        </NuxtLink>
        ...
        <NuxtLink to="/agent"
            ...
            :class="{ 'bg-[#2D3748] text-white': $route.path === '/inbox', 'justify-center': !isExpanded }">
            <div class="flex items-center">
                <Icon name="lucide:bot" class="w-6 h-6 text-gray-400 group-hover:text-white"
                    :class="{ 'text-white': $route.path === '/edit' }" />
                <span v-if="isExpanded" class="ml-3 text-base leading-6">Agent Test</span>
            </div>
            <div v-if="!isExpanded" class="absolute top-2 right-2 w-3 h-3 rounded-full bg-[#0075DC]"></div>
        </NuxtLink>
